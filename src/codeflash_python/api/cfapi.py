...
from codeflash_python.code_utils.env_utils import ensure_codeflash_api_key, get_codeflash_api_key, get_pr_number
from codeflash_python.code_utils.git_utils import get_current_branch, get_repo_owner_and_name
...
def get_cfapi_base_urls() -> BaseUrls:
    if os.environ.get("CODEFLASH_CFAPI_SERVER", "prod").lower() == "local":
        ...
        logger.info("Using local CF API at %s.", cfapi_base_url)
    else:
    ...
    *,
    ...
    try:
        ...
        if not suppress_errors:
            logger.exception(
                "CF_API_Error:: making request to Codeflash API (url: %s, method: %s, status %s): %s",
                url,
                method,
...
@lru_cache(maxsize=1)
def get_user_id(api_key: str | None = None) -> str | None:
    ...
    logger.error("Failed to look up your userid; is your CF API key valid? (%s)", response.reason)
    return None
...
def get_blocklisted_functions() -> dict[str, set[str]] | dict[str, Any]:
    ...
    try:
        ...
        if req.status_code >= 500:  # type: ignore[unsupported-operator]
            logger.error("Server error getting blocklisted functions: %s", req.status_code)
            sentry_sdk.capture_message(f"Server error in verify-existing-optimizations: {req.status_code}")
            return {}
        if not req.ok:
            if req.status_code == 401:
                logger.debug("Not authorized to check blocklisted functions for %s/%s PR #%s", owner, repo, pr_number)
            elif req.status_code == 404:
                logger.debug("PR #%s not found for %s/%s", pr_number, owner, repo)
            else:
                logger.warning("Unexpected response %s from verify-existing-optimizations", req.status_code)
            return {}

        content: dict[str, list[str]] = req.json()

        if "error" in content:
            logger.debug("No existing optimizations found for PR #%s", pr_number)
            return {}

        logger.debug("Found %s files with blocklisted functions for PR #%s", len(content), pr_number)
        return {Path(k).name: {v.replace("()", "") for v in values} for k, values in content.items()}

    except Exception as e:
        logger.exception("Error getting blocklisted functions: %s", e)
