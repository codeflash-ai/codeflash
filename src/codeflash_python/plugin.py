"""PythonPlugin — adapter wiring codeflash_python to the codeflash_core LanguagePlugin protocol."""
...
from codeflash_python.plugin_ai_ops import PluginAiOpsMixin
from codeflash_python.plugin_helpers import (
    format_code_with_ruff_or_black,
    make_test_env,
    read_return_values,
    replace_function_simple,
)
from codeflash_python.plugin_results import PluginResultsMixin
from codeflash_python.plugin_test_lifecycle import PluginTestLifecycleMixin
from codeflash_python.verification.test_runner import run_tests
...
if TYPE_CHECKING:
    ...
    from codeflash_python.context.call_graph_index import CallGraphIndex
...
class PythonPlugin(PluginAiOpsMixin, PluginTestLifecycleMixin, PluginResultsMixin):  # type: ignore[cyclic-class-definition]
    ...
    def cleanup_run(self, tests_root: Path) -> None:
        ...
        from codeflash_python.optimization.optimizer import Optimizer as PyOptimizer
        ...
        # Close the call graph index DB connection
        if self.call_graph_index and self.call_graph_index is not False:
            self.call_graph_index.close()
            self.call_graph_index = None
    ...
    def validate_environment(self, config: object) -> bool:
        from codeflash_python.code_utils.env_utils import check_formatter_installed
    ...
    def discover_functions(self, paths: list[Path]) -> list[FunctionToOptimize]:
        ...
        for path in paths:
            ...
            try:
                from codeflash_python.discovery.function_visitors import discover_functions

                internal_fns = discover_functions(source, path)
    ...
    def get_call_graph_index(self) -> CallGraphIndex | None:
        if self.call_graph_index is None:
            # Skip in CI — the cache DB doesn't persist between runs on ephemeral runners
            if os.environ.get("CI") or os.environ.get("GITHUB_ACTIONS"):
                self.call_graph_index = False
                return None
            try:
                from codeflash_python.context.call_graph_index import CallGraphIndex

                self.call_graph_index = CallGraphIndex(self.project_root)
            except Exception:
                logger.info("Failed to initialize CallGraphIndex, falling back to per-function Jedi analysis")
                self.call_graph_index = False
                return None
        return self.call_graph_index if self.call_graph_index is not False else None

    def build_index(self, files: list[Path], on_progress=None) -> None:
        graph = self.get_call_graph_index()
        if graph is not None:
            graph.build_index(files, on_progress=on_progress)
    ...
    def rank_functions(
        ...
        if trace_file and trace_file.exists():
            try:
                ...
                logger.warning("Trace-based ranking failed, falling back to dependency count")

        # Fallback: rank by dependency count (most complex first)
        graph = self.get_call_graph_index()
        if graph is None:
            return functions
        ...
        for func in functions:
            file_to_qns[func.file_path].add(func.qualified_name)
        callee_counts = graph.count_callees_per_function(dict(file_to_qns))
        self.dependency_counts = {qn: count for (_, qn), count in callee_counts.items()}
        if test_counts:
            ranked = sorted(
                enumerate(functions),
                key=lambda x: (
                    -callee_counts.get((x[1].file_path, x[1].qualified_name), 0),
            ...
            ranked = sorted(
                enumerate(functions), key=lambda x: (-callee_counts.get((x[1].file_path, x[1].qualified_name), 0), x[0])
            )
        logger.debug("Ranked %d functions by dependency count (most complex first)", len(ranked))
        return [func for _, func in ranked]
    ...
    def extract_context(self, function: FunctionToOptimize) -> CodeContext:
        from codeflash_python.context.code_context_extractor import get_code_optimization_context
        from codeflash_python.context.types import function_sources_to_helpers

        internal_fn = function
        ctx = get_code_optimization_context(internal_fn, self.project_root, call_graph=self.get_call_graph_index())
    ...
    def load_coverage(self, cov_db: Path | None, cov_config: Path | None) -> CoverageData | None:
        ...
        try:
            ...
            from codeflash_python.code_utils.config_consts import COVERAGE_THRESHOLD
