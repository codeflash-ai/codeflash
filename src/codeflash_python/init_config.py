...
class CodeflashTheme(inquirer.themes.Default):
    def __init__(self) -> None:
        super().__init__()
        self.Question.mark_color = inquirer.themes.term.yellow  # type: ignore[assignment]
        self.Question.brackets_color = inquirer.themes.term.bright_blue  # type: ignore[assignment]
        self.Question.default_color = inquirer.themes.term.bright_cyan  # type: ignore[assignment]
        self.List.selection_color = inquirer.themes.term.bright_blue  # type: ignore[assignment]
        self.Checkbox.selection_color = inquirer.themes.term.bright_blue  # type: ignore[assignment]
        self.Checkbox.selected_icon = "✅"  # type: ignore[assignment]
        self.Checkbox.unselected_icon = "⬜"  # type: ignore[assignment]
...
ignore_subdirs = {"venv", "dist", "build", "build_temp", "build_scripts", "env", "logs", "tmp", "__pycache__"}
...
def get_suggestions(section: CommonSections) -> tuple[list[str], str | None]:
...
def config_found(pyproject_toml_path: str | Path) -> tuple[bool, str]:
...
def is_valid_pyproject_toml(pyproject_toml_path: str | Path) -> tuple[bool, dict[str, Any] | None, str]:
    pyproject_toml_path = Path(pyproject_toml_path)
    try:
...
def should_modify_pyproject_toml() -> tuple[bool, dict[str, Any] | None]:
    ...
    return input(
        "A valid Codeflash config already exists in this project. Do you want to re-configure it? [y/N] "
    ).strip().lower() in ("y", "yes"), config
...
def configure_pyproject_toml(setup_info: VsCodeSetupInfo | CLISetupInfo, config_file: Path | None = None) -> bool:
...
def create_empty_pyproject_toml(pyproject_toml_path: Path) -> None:
    ...
    try:
        ...
        if pyproject_toml_path.exists() and not lsp_mode:
            print(f"Created a pyproject.toml file at {pyproject_toml_path}")
            print("Your project is now ready for Codeflash configuration!")
...
def ask_for_telemetry() -> bool:
    ...
    return input(
        "Help us improve Codeflash by sharing anonymous usage data (e.g. errors encountered)? [Y/n] "
