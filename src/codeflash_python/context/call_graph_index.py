...
if TYPE_CHECKING:
    ...
    from codeflash_python.models.call_graph import CallGraph
...
# Per-worker state, initialised by init_index_worker in child processes
worker_jedi_project: object | None = None
worker_project_root_str: str | None = None


def init_index_worker(project_root: str) -> None:
    import jedi

    global worker_jedi_project, worker_project_root_str
    worker_jedi_project = jedi.Project(path=project_root)
    worker_project_root_str = project_root


def resolve_definitions(ref: Name) -> list[Name]:
    try:
        inferred = ref.infer()
...
def is_valid_definition(definition: Name, caller_qualified_name: str, project_root_str: str) -> bool:
    ...
    try:
        from codeflash_python.context.jedi_helpers import belongs_to_function_qualified
...
def get_enclosing_function_qn(ref: Name) -> str | None:
    try:
        parent = ref.parent()
...
def analyze_file(file_path: Path, jedi_project: object, project_root_str: str) -> tuple[set[tuple[str, ...]], bool]:
    ...
    for ref in refs:
        try:
            caller_qn = get_enclosing_function_qn(ref)
            if caller_qn is None:
                continue

            definitions = resolve_definitions(ref)
            if not definitions:
                continue
            ...
            if not is_valid_definition(definition, caller_qn, project_root_str):
                continue
...
def index_file_worker(args: tuple[str, str]) -> tuple[str, str, set[tuple[str, ...]], bool]:
    """Worker entry point for ProcessPoolExecutor."""
    file_path_str, file_hash = args
    assert worker_project_root_str is not None
    edges, had_error = analyze_file(Path(file_path_str), worker_jedi_project, worker_project_root_str)
...
class CallGraphIndex:
    ...
    def __init__(self, project_root: Path, language: str = "python", db_path: Path | None = None) -> None:
        ...
        self.resolved_paths: dict[Path, str] = {}
        self.init_schema()

    def resolve_path(self, file_path: Path) -> str:
        cached = self.resolved_paths.get(file_path)
        if cached is not None:
            return cached
        resolved = str(file_path.resolve())
        self.resolved_paths[file_path] = resolved
        return resolved
    ...
    def get_callees(
        ...
        rows = cur.execute(
            ...
            WHERE ce.project_root = ? AND ce.language = ?
              AND NOT (ce.callee_file = ce.caller_file AND ce.callee_qualified_name = ce.caller_qualified_name)
              AND ce.callee_definition_type IN ('function', 'class', 'statement')
            """,
    ...
    def ensure_file_indexed(self, file_path: Path, resolved: str | None = None) -> IndexResult:
        ...
        if self.is_file_cached(resolved, file_hash):
    ...
    def index_file(self, file_path: Path, file_hash: str, resolved: str | None = None) -> IndexResult:
        if resolved is None:
            resolved = self.resolve_path(file_path)
        edges, had_error = analyze_file(file_path, self.jedi_project, self.project_root_str)
        if had_error:
            logger.debug("CallGraphIndex: failed to parse %s", file_path)
        return self.persist_edges(file_path, resolved, file_hash, edges, had_error)
    ...
    def build_index(self, file_paths: Iterable[Path], on_progress: Callable[[IndexResult], None] | None = None) -> None:
        ...
        if len(to_index) >= PARALLEL_THRESHOLD:
            self.build_index_parallel(to_index, on_progress)
        else:
            for file_path, resolved, file_hash in to_index:
                result = self.index_file(file_path, file_hash, resolved)
                self.report_progress(on_progress, result)

    def is_file_cached(self, resolved: str, file_hash: str) -> bool:
    ...
    def report_progress(self, on_progress: Callable[[IndexResult], None] | None, result: IndexResult) -> None:
    ...
    def build_index_parallel(
        ...
        logger.debug("CallGraphIndex: indexing %s files across %s workers", len(to_index), max_workers)

        try:
            with ProcessPoolExecutor(
                max_workers=max_workers, initializer=init_index_worker, initargs=(self.project_root_str,)
            ) as executor:
                futures = {executor.submit(index_file_worker, args): args[0] for args in worker_args}
                ...
                for future in as_completed(futures):
                    ...
                    try:
                        ...
                        logger.debug("CallGraphIndex: worker failed for %s", file_path)
                        self.persist_edges(file_path, resolved, file_hash, set(), had_error=True)
                        self.report_progress(
                            on_progress,
                            IndexResult(
                                file_path=file_path, cached=False, num_edges=0, edges=(), cross_file_edges=0, error=True
                    ...
                    result = self.persist_edges(file_path, resolved, file_hash, edges, had_error)
                    self.report_progress(on_progress, result)

        except Exception:
            logger.debug("CallGraphIndex: parallel indexing failed, falling back to sequential")
            self.fallback_sequential_index(to_index, on_progress)

    def fallback_sequential_index(
        self, to_index: list[tuple[Path, str, str]], on_progress: Callable[[IndexResult], None] | None
    ) -> None:
    ...
    def get_call_graph(
        ...
        from codeflash_python.models.call_graph import CallEdge, CalleeMetadata, CallGraph, FunctionNode
