...
class CommentMapper(ast.NodeVisitor):
    ...
    def visit_FunctionDef(self, node: ast.FunctionDef) -> ast.FunctionDef:
        self.process_function_def_common(node)
        return node
    ...
    def process_function_def_common(self, node: ast.FunctionDef | ast.AsyncFunctionDef) -> None:
        self.context_stack.append(node.name)
        i = len(node.body) - 1
...
def unique_inv_id(inv_id_runtimes: dict[InvocationId, list[int]], tests_project_rootdir: Path) -> dict[str, int]:
    ...
    logger.debug("[unique_inv_id] Processing %s invocation IDs", len(inv_id_runtimes))
    for inv_id, runtimes in inv_id_runtimes.items():
        ...
        if ("__unit_test_" not in abs_path_str and "__perf_test_" not in abs_path_str) or not test_qualified_name:
            logger.debug("[unique_inv_id] Skipping: path=%s, test_qualified_name=%s", abs_path_str, test_qualified_name)
            continue
        ...
        logger.debug("[unique_inv_id] Adding key: %s with runtime %s", match_key, min(runtimes))
        if match_key not in unique_inv_ids:
            unique_inv_ids[match_key] = 0
        unique_inv_ids[match_key] += min(runtimes)
    logger.debug("[unique_inv_id] Result has %s entries", len(unique_inv_ids))
...
def add_runtime_comments_to_generated_tests(
    ...
    for test in generated_tests.generated_tests:
        is_python = is_python_file(test.behavior_file_path)
        ...
        if is_python:
            ...
            try:
                ...
                logger.debug("Failed to add runtime comments to test: %s", e)
...
def remove_functions_from_generated_tests(
    ...
    function_patterns = compile_function_patterns(test_functions_to_remove)
    new_generated_tests = []
...
def compile_function_patterns(test_functions_to_remove: list[str]) -> list[re.Pattern[str]]:
    return [
        re.compile(
...
def remove_test_functions(test_source: str, functions_to_remove: list[str]) -> str:
    """Remove specific test functions from Python test source using libcst.

    Handles both bare function names (top-level) and qualified names (ClassName.method_name).
    If all test methods are removed from a class, the class is removed too.
    """
