...
class InitDecorator(ast.NodeTransformer):
    ...
    def __init__(
        ...
        self.attrs_classes_to_patch: dict[str, ast.Call] = {}
        ...
        self.base_decorator_func = ast.Name(id="codeflash_capture", ctx=ast.Load())

        # Preconstruct starred/kwargs for super init injection for perf
        self.super_starred = ast.Starred(value=ast.Name(id="args", ctx=ast.Load()))
        self.super_kwarg = ast.keyword(arg=None, value=ast.Name(id="kwargs", ctx=ast.Load()))
        self.super_func = ast.Attribute(
            value=ast.Call(func=ast.Name(id="super", ctx=ast.Load()), args=[], keywords=[]),
            attr="__init__",
            ctx=ast.Load(),
        )
        self.init_vararg = ast.arg(arg="args")
        self.init_kwarg = ast.arg(arg="kwargs")
        self.init_self_arg = ast.arg(arg="self", annotation=None)
        ...
        self.super_call_expr = ast.Expr(
            value=ast.Call(func=self.super_func, args=[self.super_starred], keywords=[self.super_kwarg])
        )
        # Create function arguments: self, *args, **kwargs (reuse arg nodes)
        self.init_arguments = ast.arguments(
            posonlyargs=[],
            args=[self.init_self_arg],
            vararg=self.init_vararg,
            kwonlyargs=[],
            kw_defaults=[],
            kwarg=self.init_kwarg,
            defaults=[],
        )

        # Pre-build reusable AST nodes for build_attrs_patch_block
        self.load_ctx = ast.Load()
        self.store_ctx = ast.Store()
        self.args_name_load = ast.Name(id="args", ctx=self.load_ctx)
        self.kwargs_name_load = ast.Name(id="kwargs", ctx=self.load_ctx)
        self.self_arg_node = ast.arg(arg="self")
        self.args_arg_node = ast.arg(arg="args")
        self.kwargs_arg_node = ast.arg(arg="kwargs")
        self.self_name_load = ast.Name(id="self", ctx=self.load_ctx)
        self.starred_args = ast.Starred(value=self.args_name_load, ctx=self.load_ctx)
        self.kwargs_keyword = ast.keyword(arg=None, value=self.kwargs_name_load)

    def visit_ImportFrom(self, node: ast.ImportFrom) -> ast.ImportFrom:
        # Check if our import already exists
        if node.module == "codeflash_python.verification.codeflash_capture" and any(
    ...
    def visit_Module(self, node: ast.Module) -> ast.Module:
        ...
        if self.attrs_classes_to_patch:
            new_body: list[ast.stmt] = []
            for stmt in node.body:
                new_body.append(stmt)
                if isinstance(stmt, ast.ClassDef) and stmt.name in self.attrs_classes_to_patch:
                    new_body.extend(self.build_attrs_patch_block(stmt.name, self.attrs_classes_to_patch[stmt.name]))
            node.body = new_body

        # Add import statement
        if not self.has_import and self.inserted_decorator:
            import_stmt = ast.parse(
                "from codeflash_python.verification.codeflash_capture import codeflash_capture"
            ).body[0]
    ...
    def visit_ClassDef(self, node: ast.ClassDef) -> ast.ClassDef:
        ...
        if not has_init:
                        ...
                        if isinstance(dec, ast.Call):
                            for kw in dec.keywords:
                                if kw.arg == "init" and isinstance(kw.value, ast.Constant) and kw.value.value is False:
                                    return node
                        self.attrs_classes_to_patch[node.name] = decorator
                        self.inserted_decorator = True
                        return node

            # Create super().__init__(*args, **kwargs) call (use prebuilt AST fragments)
            super_call = self.super_call_expr
            ...
            init_func = ast.FunctionDef(
                name="__init__", args=self.init_arguments, body=[super_call], decorator_list=[decorator], returns=None
            )
    ...
    def build_attrs_patch_block(self, class_name: str, decorator: ast.Call) -> list[ast.stmt]:
        orig_name = f"_codeflash_orig_{class_name}_init"
        patched_name = f"_codeflash_patched_{class_name}_init"

        class_name_load = ast.Name(id=class_name, ctx=self.load_ctx)

        # _codeflash_orig_ClassName_init = ClassName.__init__
        save_orig = ast.Assign(
            targets=[ast.Name(id=orig_name, ctx=self.store_ctx)],
            value=ast.Attribute(value=class_name_load, attr="__init__", ctx=self.load_ctx),
        )
        ...
        patched_func = ast.FunctionDef(
            ...
            args=ast.arguments(
                posonlyargs=[],
                args=[self.self_arg_node],
                vararg=self.args_arg_node,
                kwonlyargs=[],
                kw_defaults=[],
                kwarg=self.kwargs_arg_node,
                defaults=[],
            ),
            body=[
                ast.Return(
                    value=ast.Call(
                        func=ast.Name(id=orig_name, ctx=self.load_ctx),
                        args=[self.self_name_load, self.starred_args],
                        keywords=[self.kwargs_keyword],
                    )
                )
            ],
            decorator_list=[],
        ...
        assign_patched = ast.Assign(
            targets=[
                ast.Attribute(value=ast.Name(id=class_name, ctx=self.load_ctx), attr="__init__", ctx=self.store_ctx)
            ],
            value=ast.Call(func=decorator, args=[ast.Name(id=patched_name, ctx=self.load_ctx)], keywords=[]),
        )

        return [save_orig, patched_func, assign_patched]

    def expr_name(self, node: ast.AST) -> str | None:
        if isinstance(node, ast.Name):
            return node.id
        if isinstance(node, ast.Call):
            return self.expr_name(node.func)
        if isinstance(node, ast.Attribute):
            parent = self.expr_name(node.value)
