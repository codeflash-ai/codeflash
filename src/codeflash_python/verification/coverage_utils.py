...
class CoverageUtils:
    ...
    def load_from_sqlite_database(
        ...
        if not database_path.exists() or not database_path.stat().st_size:
            logger.debug("Coverage database %s is empty or does not exist", database_path)
        ...
        coverage_data, status = CoverageUtils.parse_coverage_file(temp_json_file, source_code_path)

        main_func_coverage, dependent_func_coverage = CoverageUtils.fetch_function_coverages(
            function_name, code_context, coverage_data, original_cov_data=original_coverage_data
        )

        total_executed_lines, total_unexecuted_lines = CoverageUtils.aggregate_coverage(
        ...
        graph = CoverageUtils.build_graph(main_func_coverage, dependent_func_coverage)
    ...
    def parse_coverage_file(
        ...
        logger.debug("Looking for coverage data in %s", " -> ".join(candidates))
        for candidate in candidates:
            try:
                cov: dict[str, dict[str, Any]] = coverage_data["files"][candidate]["functions"]
                logger.debug("Coverage data found for %s in %s", source_code_path, candidate)
                status = CoverageStatus.PARSED_SUCCESSFULLY
                break
            except KeyError:
                continue
        else:
            logger.debug("No coverage data found for %s in %s", source_code_path, candidates)
            cov = {}
            status = CoverageStatus.NOT_FOUND
        return cov, status

    @staticmethod
    def fetch_function_coverages(
        function_name: str,
        code_context: CodeOptimizationContext,
