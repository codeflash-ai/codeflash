...
def apply_deterministic_patches() -> None:
    ...
    _original_datetime_utcnow = datetime.datetime.utcnow  # type: ignore[attr-defined]
    ...
    def mock_datetime_utcnow() -> datetime.datetime:
        ...
        _original_datetime_utcnow()  # type: ignore[attr-defined]  # Maintain performance characteristics
        return fixed_datetime
    ...
    # Apply patches
    time.time = mock_time_time  # type: ignore[misc]
    time.perf_counter = mock_perf_counter  # type: ignore[misc]
    uuid.uuid4 = mock_uuid4  # type: ignore[misc]
    uuid.uuid1 = mock_uuid1  # type: ignore[misc]

    # Seed random module for other random functions
    random.seed(42)
    random.random = mock_random  # type: ignore[method-assign]
    ...
    builtins._original_datetime_now = _original_datetime_now  # type: ignore[attr-defined]  # noqa: SLF001
    builtins._original_datetime_utcnow = _original_datetime_utcnow  # type: ignore[attr-defined]  # noqa: SLF001
    builtins._mock_datetime_now = mock_datetime_now  # type: ignore[attr-defined]  # noqa: SLF001
    builtins._mock_datetime_utcnow = mock_datetime_utcnow  # type: ignore[attr-defined]  # noqa: SLF001
    ...
    try:
        ...
        os.urandom = mock_urandom  # type: ignore[method-assign]
...
class PytestLoops:
    ...
    def __init__(self, config: Config) -> None:
        ...
        self.module_clearables: dict[str, list[Callable]] = {}
    ...
    def pytest_runtestloop(self, session: Session) -> bool:
        ...
        total_time: float = self.get_total_time(session)
        ...
        while total_time >= SHORTEST_AMOUNT_OF_TIME:  # need to run at least one for normal tests
            ...
            for index, item in enumerate(session.items):
                ...
                if total_time > SHORTEST_AMOUNT_OF_TIME:
                    item._nodeid = self.set_nodeid(item._nodeid, count)  # noqa: SLF001

                next_item: pytest.Item | None = session.items[index + 1] if index + 1 < len(session.items) else None

                self.clear_lru_caches(item)
            ...
            if self.timed_out(session, start_time, count):
                break

            _ORIGINAL_TIME_SLEEP(self.get_delay_time(session))
        return True

    def clear_lru_caches(self, item: pytest.Item) -> None:
        ...
        try:
            clearables = self.module_clearables.get(module_name)
            if clearables is None:
                clearables = self.scan_module_clearables(module_name)
                self.module_clearables[module_name] = clearables
    ...
    def scan_module_clearables(self, module_name: str) -> list[Callable]:
    ...
    def set_nodeid(self, nodeid: str, count: int) -> str:
    ...
    def get_delay_time(self, session: Session) -> float:
    ...
    def get_total_time(self, session: Session) -> float:
    ...
    def timed_out(self, session: Session, start_time: float, count: int) -> bool:
        ...
        return count >= session.config.option.codeflash_max_loops or (
            ...
            and _ORIGINAL_TIME_TIME() - start_time > self.get_total_time(session)
        )
