...
from codeflash_python.picklepatch.pickle_placeholder import PicklePlaceholderAccessError
...
if HAS_PYRSISTENT:
    import pyrsistent  # type: ignore[import-not-found]
...
def extract_exception_from_message(msg: str) -> BaseException | None:
...
def get_wrapped_exception(exc: BaseException) -> BaseException | None:
    ...
    if hasattr(exc, "exceptions"):
        ...
        if len(exceptions) == 1:  # type: ignore[arg-type]
            return exceptions[0]  # type: ignore[index]
    ...
    return extract_exception_from_message(str(exc))
...
def comparator(orig: Any, new: Any, superset_obj: bool = False) -> bool:
    ...
    try:
        ...
        if isinstance(orig, str):
            ...
            if is_temp_path(orig) and is_temp_path(new):
                return normalize_temp_path(orig) == normalize_temp_path(new)
            return False
        ...
        if str(type(orig)) == "<class 'object'>":
            return True
        logger.warning("Unknown comparator input type: %s", type(orig))
        sentry_sdk.capture_exception(RuntimeError(f"Unknown comparator input type: {type(orig)}"))
        return False
    except RecursionError as e:
        logger.exception("RecursionError while comparing objects: %s", e)
        sentry_sdk.capture_exception(e)
        return False
    except Exception as e:
        logger.exception("Error while comparing objects: %s", e)
        sentry_sdk.capture_exception(e)
        return False
