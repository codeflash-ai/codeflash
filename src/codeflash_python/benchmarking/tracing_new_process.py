...
from codeflash_python.benchmarking.tracing_utils import (
    FunctionModules,
    filter_files_optimized,
    module_name_from_file_path,
)
from codeflash_python.picklepatch.pickle_patcher import PicklePatcher
...
def patch_ap_scheduler() -> None:
    if find_spec("apscheduler"):
        import apscheduler.schedulers.background as bg  # type: ignore[unresolved-import]
        import apscheduler.schedulers.blocking as bb  # type: ignore[unresolved-import]
        from apscheduler.schedulers import base  # type: ignore[unresolved-import]
...
class Tracer:
    ...
    def __init__(
        ...
        *,
        ...
        if os.environ.get("CODEFLASH_TRACER_DISABLE", "0") == "1":
            logger.warning("Codeflash: Tracer disabled by environment variable CODEFLASH_TRACER_DISABLE")
            disable = True
        ...
        if sys.getprofile() is not None or sys.gettrace() is not None:
            logger.warning(
                "Codeflash: Another profiler, debugger or coverage tool is already running. "
        ...
        try:
            ...
            # If verification_utils doesn't exist yet, create a simple fallback
            def get_test_file_path(test_dir: Path, function_name: str, test_type: str) -> Path:
                return test_dir / "codeflash_replay_tests" / f"test_{function_name}_{test_type}.py"
    ...
    def __enter__(self) -> None:
        ...
        if Path(self.output_file).exists():
            logger.info("Removing existing trace file")
        ...
        logger.info("Codeflash: Traced Program Output Begin")
    ...
    def __exit__(
        ...
        with self._db_lock:
            ...
            logger.info("Codeflash: Traced Program Output End")
        ...
        try:
            from codeflash_python.benchmarking.replay_test import (
                create_trace_replay_test,  # type: ignore[unresolved-import]
            )
            from codeflash_python.verification.verification_utils import get_test_file_path
        except ImportError:
            # If modules don't exist yet, create minimal fallbacks
            def create_trace_replay_test(trace_file: Path, functions: list, max_run_count: int) -> str:
                return f"# Replay test for {trace_file}\n# Functions: {[f.function_name for f in functions]}\n"
        ...
        logger.info(
            "Codeflash: Traced %d function calls successfully and replay test created at - %s",
            self.trace_count,
            test_file_path,
        )
    ...
    def tracer_logic(self, frame: FrameType, event: str) -> None:
        if event != "call":
            return
        if self.start_time is not None and self.timeout is not None and (time.time() - self.start_time) > self.timeout:
            sys.setprofile(None)
            threading.setprofile(None)
            logger.warning("Codeflash: Timeout reached! Stopping tracing at %s seconds.", self.timeout)
            return
        ...
        co_qualname = getattr(code, "co_qualname", "")
        if class_name is None and "." in co_qualname:
            qualname_parts = co_qualname.split(".")
            if len(qualname_parts) >= 2:
                class_name = qualname_parts[-2]

        try:
            function_qualified_name = f"{file_name}:{co_qualname}"
        except AttributeError:
    ...
    def trace_callback(self, frame: FrameType, event: str, arg: str | None) -> None:
        ...
        if event == "c_call" and arg is not None:
            self.c_func_name = arg.__name__  # type: ignore[union-attr]
    ...
    def simulate_call(self, name: str) -> None:
        ...
        pframe_raw = self.cur[-2] if self.cur else None
        pframe: FakeFrame | None = pframe_raw if isinstance(pframe_raw, FakeFrame) else None
        frame = FakeFrame(code, pframe)
        self.dispatch["call"](self, frame, 0)  # type: ignore[arg-type]
    ...
    def print_stats(self, sort: str | int | tuple = -1) -> None:
        if not self.stats:
            logger.info("Codeflash: No stats available to print")
            self.total_tt = 0
            return
        ...
        try:
            ...
            for func, stats_data in stats_items:
                try:
                    ...
                    if len(stats_data) != 5:
                        logger.debug("Skipping malformed stats data for %s: %s", func, stats_data)
                        continue
                    ...
                    for caller_func, count in callers_items:
                        if isinstance(caller_func, tuple):
                            ...
                            logger.debug("Unexpected caller format: %s", caller_func)
                    ...
                    logger.debug("Error converting stats for %s: %s", func, e)
                    continue
            ...
            for func, timing_data in timings_items:
                try:
                    if len(timing_data) != 5:
                        logger.debug("Skipping malformed timing data for %s: %s", func, timing_data)
                        continue
                    ...
                    logger.debug("Error converting timings for %s: %s", func, e)
                    continue
            ...
            logger.info(
                "%d function calls (%d primitive calls) in %.3f milliseconds",
                total_calls,
                total_primitive,
                self.total_tt / 1e6,
            )
            ...
            for func, (cc, nc, tt, ct, _) in sorted_stats:
                ...
                calls_str = f"{cc}/{nc}" if cc != nc else str(cc)
                tt_ms = tt / 1e6
                ct_ms = ct / 1e6
                per_call = tt_ms / cc if cc > 0 else 0
                cum_per_call = ct_ms / nc if nc > 0 else 0
                logger.debug(
                    "  %s  %7.3fms  %7.3fms/call  %7.3fms cum  %7.3fms/call  %s (%s:%d)",
                    calls_str,
                    tt_ms,
                    per_call,
                    ct_ms,
            ...
            logger.warning("Error in stats processing: %s", e)
            logger.info("Traced %d function calls", self.trace_count)
            self.total_tt = 0
...
if __name__ == "__main__":
    ...
    # code is either str (for module) or CodeType (for file), but runctx expects str
    # When code is CodeType, exec() in runctx will handle it correctly
    tracer.runctx(code, globs, {})  # type: ignore[arg-type]
