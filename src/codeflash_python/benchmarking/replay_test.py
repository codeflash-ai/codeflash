...
from codeflash_python.discovery.function_visitors import inspect_top_level_functions_or_methods
...
def create_trace_replay_test_code(
    ...
    """
    # Create Imports
    imports = """from codeflash_python.picklepatch.pickle_patcher import PicklePatcher as pickle
from codeflash_python.benchmarking.replay_test import get_next_arg_and_return
"""
...
"""
    ...
    for func in functions_data:
        ...
        function_name = func.get("function_name")
        class_name = func.get("class_name")
        file_path_raw = func.get("file_path")
        benchmark_function_name = func.get("benchmark_function_name")
        function_properties = func.get("function_properties")
        assert module_name is not None
        assert function_name is not None
        assert file_path_raw is not None
        assert benchmark_function_name is not None
        assert function_properties is not None
        file_path = Path(file_path_raw).as_posix()
        if not class_name:
    ...
    try:
        ...
        for benchmark_file in benchmark_files:
            ...
            if not functions_data:
                logger.info("No benchmark test functions found in %s", benchmark_module_path)
                continue
        ...
        logger.info("Error generating replay tests: %s", e)
