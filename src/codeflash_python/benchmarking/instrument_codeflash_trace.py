            ...
            body=[
                cst.ImportFrom(
                    module=cst.Attribute(
                        value=cst.Attribute(
                            value=cst.Name(value="codeflash_python"), attr=cst.Name(value="benchmarking")
                        ),
...
def instrument_codeflash_trace_decorator(file_to_funcs_to_optimize: dict[Path, list[FunctionToOptimize]]) -> None:
    ...
    for file_path, functions_to_optimize in file_to_funcs_to_optimize.items():
        ...
        skip = False
        for pkg_marker in ("/codeflash_python/", "/codeflash/"):
            _, sep, after = posix_path.rpartition(pkg_marker)
            if sep:
