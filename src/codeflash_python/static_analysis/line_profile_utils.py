...
class JitDecoratorDetector(ast.NodeVisitor):
    ...
    def check_name_decorator(self, name: str) -> bool:
        ...
        return self.is_known_jit_decorator(module, imported_name)

    def check_attribute_decorator(self, node: ast.Attribute) -> bool:
        """Check if an attribute decorator (e.g., @numba.jit) is a JIT decorator."""
        # Build the full attribute chain
        parts = self.get_attribute_parts(node)
        if not parts:
            return False
        ...
        if first_part in self.import_aliases:
            ...
            if imported_name is None:
                ...
                if rest_parts:
                    ...
                    return self.is_known_jit_decorator(full_module, decorator_name)
    ...
    def get_attribute_parts(self, node: ast.Attribute) -> list[str]:
    ...
    def is_known_jit_decorator(self, module: str, decorator_name: str) -> bool:
...
class LineProfilerDecoratorAdder(cst.CSTTransformer):
    ...
    def leave_FunctionDef(self, original_node: cst.FunctionDef, updated_node: cst.FunctionDef) -> cst.FunctionDef:
        ...
        if self.context_stack == self.qualified_name_parts:
            ...
            has_decorator = any(self.is_target_decorator(decorator.decorator) for decorator in original_node.decorators)  # type: ignore[arg-type]
...
class ProfileEnableTransformer(cst.CSTTransformer):
    ...
    def leave_ImportFrom(self, original_node: cst.ImportFrom, updated_node: cst.ImportFrom) -> cst.ImportFrom:
        ...
        if (
            ...
            and not isinstance(original_node.names, cst.ImportStar)
            and any(
                name.name.value == "profile" and (not name.asname or name.asname.name.value == "codeflash_line_profile")  # type: ignore[attr-defined]
            ...
            if hasattr(original_node, "leading_lines"):
                leading_whitespace = original_node.leading_lines[-1].whitespace if original_node.leading_lines else ""  # type: ignore[index,attr-defined]
                ...
                for small_stmt in stmt.body:
                    if isinstance(small_stmt, cst.ImportFrom) and (
                        ...
                        and any(
                            ...
                            and (not name.asname or name.asname.name.value == "codeflash_line_profile")  # type: ignore[attr-defined]
