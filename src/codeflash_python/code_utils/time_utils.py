...
def format_time(nanoseconds: int) -> str:
    ...
    if nanoseconds < 1_000_000:
        ...
        return (
            f"{value:.2f}\u03bcs" if value < 10 else (f"{value:.1f}\u03bcs" if value < 100 else f"{int(value)}\u03bcs")
        )
...
def format_runtime_comment(original_time_ns: int, optimized_time_ns: int, comment_prefix: str = "#") -> str:
    perf_gain = format_perf(
        abs(((original_time_ns - optimized_time_ns) / optimized_time_ns) * 100) if optimized_time_ns else 0.0
    )
