...
def is_powershell() -> bool:
    ...
    if "powershell" in comspec:
        logger.debug("shell_utils.py:is_powershell - Detected PowerShell via COMSPEC: %s", comspec)
        return True
    ...
    if "windows" in term_program and "terminal" in term_program and "cmd.exe" not in comspec:
        logger.debug("shell_utils.py:is_powershell - Detected PowerShell via Windows Terminal (COMSPEC: %s)", comspec)
        return True

    logger.debug("shell_utils.py:is_powershell - Not PowerShell (COMSPEC: %s, TERM_PROGRAM: %s)", comspec, term_program)
    return False
...
def read_api_key_from_shell_config() -> str | None:
    ...
    try:
        ...
        with open(shell_rc_path_str, encoding="utf8") as shell_rc:
            shell_contents = shell_rc.read()
            matches = pattern.findall(shell_contents)
            if matches:
                logger.debug("shell_utils.py:read_api_key_from_shell_config - Found API key in file: %s", shell_rc_path)
                return matches[-1]
            logger.debug("shell_utils.py:read_api_key_from_shell_config - No API key found in file: %s", shell_rc_path)
            return None
    except FileNotFoundError:
        logger.debug("shell_utils.py:read_api_key_from_shell_config - File not found: %s", shell_rc_path)
        return None
    except Exception as e:
        logger.debug("shell_utils.py:read_api_key_from_shell_config - Error reading file: %s", e)
        return None
...
def save_api_key_to_rc(api_key: str) -> Result[str, str]:
    ...
    logger.debug("shell_utils.py:save_api_key_to_rc - Saving API key to: %s", shell_rc_path)
    logger.debug("shell_utils.py:save_api_key_to_rc - API key line format: %s...", api_key_line[:30])
    ...
    try:
        ...
        try:
            with open(shell_rc_path_str, "r+", encoding="utf8") as shell_file:
                ...
                logger.debug("shell_utils.py:save_api_key_to_rc - Read existing file, length: %s", len(shell_contents))
                ...
                logger.debug("shell_utils.py:save_api_key_to_rc - Existing key in file: %s", existing_in_file)
        ...
        logger.debug("shell_utils.py:save_api_key_to_rc - Successfully wrote to %s", shell_rc_path)

        return Ok(f"✅ {action} {shell_rc_path}")
    except PermissionError as e:
        logger.debug("shell_utils.py:save_api_key_to_rc - Permission error: %s", e)
        return Err(
        ...
        logger.debug("shell_utils.py:save_api_key_to_rc - Error: %s", e)
        return Err(
...
def get_cross_platform_subprocess_run_args(
    ...
    run_args: dict[str, object] = {"cwd": cwd, "env": env, "text": text, "timeout": timeout, "check": check}
