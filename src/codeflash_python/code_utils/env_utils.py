...
from codeflash_python.code_utils.formatter import format_code
from codeflash_python.code_utils.shell_utils import read_api_key_from_shell_config, save_api_key_to_rc
...
def check_formatter_installed(
    ...
    if shutil.which(exe_name) is None:
        logger.error(
            "Could not find formatter: %s\nPlease install it or update 'formatter-cmds' in your codeflash configuration",
            command_str,
        )
        return False
    ...
    try:
        ...
        logger.exception("Formatter failed to run: %s\nError: %s", command_str, e)
        return False
...
def get_codeflash_api_key() -> str:
    ...
    logger.debug(
        "env_utils.py:get_codeflash_api_key - env_api_key: %s, shell_api_key: %s",
        "***" + env_api_key[-4:] if env_api_key else None,
        "***" + shell_api_key[-4:] if shell_api_key else None,
    )
    ...
    if env_api_key and not shell_api_key:
        try:
            ...
            if result.is_ok():
                logger.debug(
                    "env_utils.py:get_codeflash_api_key - Automatically saved API key from environment to shell config: %s",
                    result.unwrap(),
                )
            else:
                logger.debug("env_utils.py:get_codeflash_api_key - Failed to save API key: %s", result.error)  # type: ignore[unresolved-attribute]
        except Exception as e:
            logger.debug(
                "env_utils.py:get_codeflash_api_key - Failed to automatically save API key to shell config: %s", e
            )
