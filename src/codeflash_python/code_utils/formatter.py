...
def apply_formatter_cmds(
    cmds: list[str], path: Path, test_dir_str: str | None, print_status: bool, exit_on_failure: bool = True
) -> tuple[Path, str, bool]:
    if not path.exists():
    ...
    for command in cmds:
        ...
        try:
            ...
            if result.returncode == 0:
                if print_status:
                    logger.info("Formatted Successfully with: %s", command.replace("$file", path.name))
                changed = True
            else:
                logger.error("Failed to format code with %s", " ".join(formatter_cmd_list))
        except FileNotFoundError as e:
            command_str = " ".join(str(part) for part in formatter_cmd_list)
            logger.warning("Formatter command not found: %s", command_str)
            if exit_on_failure:
                raise e from None
...
def format_code(
    ...
    with tempfile.TemporaryDirectory() as test_dir_str:
        ...
        if check_diff and original_code_lines > 50:
            ...
            if not changed:
                logger.warning(
                    "No changes detected in %s after formatting, are you sure you have valid formatter commands?", path
                )
            ...
            if diff_lines_count > max_diff_lines:
                logger.warning(
                    "Skipping formatting %s: %s lines would change (max: %s)", path, diff_lines_count, max_diff_lines
                )
                return original_code
        ...
        logger.debug("Formatted %s with commands: %s", path, formatter_cmds)
        return formatted_code
