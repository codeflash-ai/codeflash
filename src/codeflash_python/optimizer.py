...
def prepare_python_module(
    ...
    """
    from codeflash_python.static_analysis.code_replacer import normalize_code, normalize_node
    from codeflash_python.static_analysis.static_analysis import analyze_imported_modules

    try:
        original_module_ast = ast.parse(original_module_code)
    except SyntaxError as e:
        logger.warning("Syntax error parsing code in %s: %s", original_module_path, e)
    ...
    for analysis in imported_module_analyses:
        ...
        try:
            ...
            logger.warning("Syntax error parsing code in callee module %s: %s", analysis.file_path, e)
...
def resolve_python_function_ast(
    ...
    from codeflash_python.static_analysis.static_analysis import get_first_top_level_function_or_method_ast
