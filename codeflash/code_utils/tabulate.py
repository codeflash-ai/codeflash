...
def _visible_width(s):
    if wcwidth is not None and WIDE_CHARS_MODE:
        if isinstance(s, (str, bytes)):
            return wcwidth.wcswidth(_strip_ansi(s))
...
def _choose_width_fn(has_invisible, enable_widechars, is_multiline):
    if is_multiline:
        if has_invisible:
            return lambda s: _multiline_width(s, _visible_width)
        if enable_widechars:
            return lambda s: _multiline_width(s, wcwidth.wcswidth)
        return lambda s: _multiline_width(s, len)
    if has_invisible:
