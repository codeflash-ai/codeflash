...
def _is_separating_line(row):
    if isinstance(row, (list, str)):
        return (
            _is_separating_line_value(row[0])
            if len(row) >= 1
            else False or _is_separating_line_value(row[1])
            if len(row) >= 2
            else False
        )
    return False
...
def _format_table(fmt, headers, headersaligns, rows, colwidths, colaligns, is_multiline, rowaligns):
    lines = []
    hidden = fmt.with_header_hide if headers else []
    pad = fmt.padding
    ...
    if padded_headers:
        append_row(lines, padded_headers, padded_widths, headersaligns, fmt.headerrow)
