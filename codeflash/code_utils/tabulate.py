...
def _align_column_choose_padfn(strings, alignment, has_invisible, preserve_whitespace):
    if alignment == "right":
        strings = _prepare_strings(strings, alignment, preserve_whitespace)
        padfn = _padleft
...
# Helper function for choosing pad function and pre-processing strings
def _prepare_strings(strings, alignment, preserve_whitespace, process_all=False):
    if not preserve_whitespace or process_all:
