...
def _isnumber_with_thousands_separator(string):
    return _isnumber_with_thousands_separator_cached(string)
...
def _isnumber(string):
    return _isnumber_cached(string)


def _isint(string, inttype=int):
    return _isint_cached(string, inttype)
...
@cache
def _isnumber_with_thousands_separator_cached(string):
    try:
...
@cache
def _isconvertible_cached(conv, string):
    try:
        conv(string)
        return True
    except (ValueError, TypeError):
...
@cache
def _isint_cached(string, inttype=int):
    return (
