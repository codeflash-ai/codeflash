...
def _build_row(padded_cells, colwidths, colaligns, rowfmt):
    ...
    # Directly call the simplified row builder for fixed format
    begin, sep, end = rowfmt
    return (begin + sep.join(padded_cells) + end).rstrip()
...
def _append_basic_row(lines, padded_cells, colwidths, colaligns, rowfmt, rowalign=None):
    ...
    if rowfmt:
        if callable(rowfmt):
            lines.append(rowfmt(padded_cells, colwidths, colaligns))
        else:
            begin, sep, end = rowfmt
            lines.append((begin + sep.join(padded_cells) + end).rstrip())
    return lines
