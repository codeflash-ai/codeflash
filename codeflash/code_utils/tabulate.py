...
def _more_generic(type1, type2):
    # Define the type ranking directly in a tuple in the order from least to most generic
    types = (
        type(None),  # 0
        bool,  # 1
        int,  # 2
    ...
    # Default to the highest ranking (most generic), which is 'str'
    rank1 = types.index(type1) if type1 in types else 5
    rank2 = types.index(type2) if type2 in types else 5

    # Return the more generic type
    return types[max(rank1, rank2)]
