...
def _is_separating_line(row):
    return (isinstance(row, (list, str)) and len(row) >= 1 and _is_separating_line_value(row[0])) or (
        isinstance(row, (list, str)) and len(row) >= 2 and _is_separating_line_value(row[1])
    )
...
def _remove_separating_lines(rows):
    if isinstance(rows, list):
        ...
        for index, row in enumerate(rows):
            if isinstance(row, (list, str)) and (
