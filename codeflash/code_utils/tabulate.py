...
def _pipe_segment_with_colons(align, colwidth):
    ...
    if align in {"right", "decimal"}:
        # Left pad with dashes, right ends with colon
        return ("-" * (w - 1)) + ":"
    elif align == "center":
        # Both sides have colons, center filled with dashes
        return ":" + ("-" * (w - 2)) + ":"
    elif align == "left":
        # Left start with colon, rest dashes
        return ":" + ("-" * (w - 1))
    else:
        # No alignment info: all dashes
        return "-" * w
...
def _pipe_line_with_colons(colwidths, colaligns):
    ...
    """
    # Avoid attribute lookups inside the loop, localize fastpaths
    join = "|".join

    if not colaligns:  # e.g. printing an empty data frame (github issue #15)
        colaligns = [""] * len(colwidths)
    # Avoid repeated name lookups and use list comprehension for speed
