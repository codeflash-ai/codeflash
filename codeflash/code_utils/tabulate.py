...
def _isint(string, inttype=int):
    # Directly check for native int type
    if type(string) is inttype:
        return True

    # Check for numpy integer types using numpy functions
    if isinstance(string, np.integer):
        return True

    # Check for str or bytes representations of integers
    if isinstance(string, (bytes, str)):
        return _isconvertible(inttype, string)
