...
# Embedded plugin source for PyInstaller binaries (constants already inlined)
# This string is used when running in PyInstaller mode where inspect.getsource() doesn't work
_EMBEDDED_PLUGIN_SOURCE = '''# ruff: noqa
from __future__ import annotations
...
# Inlined constants for PyInstaller
STABILITY_CENTER_TOLERANCE = 0.0025
STABILITY_SPREAD_TOLERANCE = 0.0025
STABILITY_WINDOW_SIZE = 0.35
...
class PytestLoops:
    ...
    def _clear_lru_caches(self, item: pytest.Item) -> None:
        ...
        _clear_cache_for_object(item.function)

        try:
            if hasattr(item.function, "__module__"):
...
def get_embedded_plugin_source() -> str:
    """Get standalone plugin source with inlined constants for PyInstaller."""
    return _EMBEDDED_PLUGIN_SOURCE
