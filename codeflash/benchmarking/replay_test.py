...
def get_next_arg_and_return(
    trace_file: str, function_name: str, file_name: str, class_name: str | None, num_to_get: int = 25
) -> Generator[Any]:
    ...
    if class_name is not None:
        cursor = cur.execute(
            "SELECT * FROM function_calls WHERE function_name = ? AND file_namename = ? AND class_name = ? ORDER BY time_ns ASC LIMIT ?",
            (function_name, file_name, class_name, limit),
        )
    else:
        cursor = cur.execute(
            "SELECT * FROM function_calls WHERE function_name = ? AND file_namename = ? ORDER BY time_ns ASC LIMIT ?",
