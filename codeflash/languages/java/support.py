...
class JavaSupport(LanguageSupport):
    ...
    def instrument_existing_test(
        ...
        For replay test files (generated by the tracer), skip instrumentation —
        they call helper.replay() via reflection, not the target function directly.
        The behavior instrumentation can't wrap indirect calls and produces
        malformed output for large replay test files.
        """
        test_string = test_path.read_text(encoding="utf-8")

        # Skip instrumentation for replay tests — just rename the class
        if test_string.lstrip().startswith("// codeflash:"):
            import re

            original_class = test_path.stem
            if mode == "behavior":
                new_class = f"{original_class}__perfinstrumented"
            else:
                new_class = f"{original_class}__perfonlyinstrumented"
            modified = re.sub(rf"\b{re.escape(original_class)}\b", new_class, test_string)
