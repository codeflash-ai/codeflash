...
def _generate_sqlite_write_code(
    ...
    """
    inner_indent = indent + "    "
    suffix = f"{iter_id}_{call_counter}"
    id_only = f"{iter_id}"
    call_only = f"{call_counter}"

    return [
        f"{indent}}} finally {{",
        f"{inner_indent}long _cf_end{suffix}_finally = System.nanoTime();",
        f"{inner_indent}long _cf_dur{suffix} = (_cf_end{suffix} != -1 ? _cf_end{suffix} : _cf_end{suffix}_finally) - _cf_start{suffix};",
        f'{inner_indent}System.out.println("!######" + _cf_mod{id_only} + ":" + _cf_cls{id_only} + "." + _cf_test{id_only} + ":" + _cf_fn{id_only} + ":" + _cf_loop{id_only} + ":" + "{call_only}" + "######!");',
        f"{inner_indent}// Write to SQLite if output file is set",
        f"{inner_indent}if (_cf_outputFile{id_only} != null && !_cf_outputFile{id_only}.isEmpty()) {{",
        f"{inner_indent}    try {{",
        f'{inner_indent}        Class.forName("org.sqlite.JDBC");',
        f'{inner_indent}        try (Connection _cf_conn{suffix} = DriverManager.getConnection("jdbc:sqlite:" + _cf_outputFile{id_only})) {{',
        f"{inner_indent}            try (java.sql.Statement _cf_stmt{suffix} = _cf_conn{suffix}.createStatement()) {{",
        f'{inner_indent}                _cf_stmt{suffix}.execute("CREATE TABLE IF NOT EXISTS test_results (" +',
        ...
        f'{inner_indent}                    "runtime INTEGER, return_value BLOB, verification_type TEXT, stdout TEXT)");',
        f"{inner_indent}            }}",
        f'{inner_indent}            String _cf_sql{suffix} = "INSERT INTO test_results VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";',
        f"{inner_indent}            try (PreparedStatement _cf_pstmt{suffix} = _cf_conn{suffix}.prepareStatement(_cf_sql{suffix})) {{",
        f"{inner_indent}                _cf_pstmt{suffix}.setString(1, _cf_mod{id_only});",
        f"{inner_indent}                _cf_pstmt{suffix}.setString(2, _cf_cls{id_only});",
        f"{inner_indent}                _cf_pstmt{suffix}.setString(3, _cf_test{id_only});",
        f"{inner_indent}                _cf_pstmt{suffix}.setString(4, _cf_fn{id_only});",
        f"{inner_indent}                _cf_pstmt{suffix}.setInt(5, _cf_loop{id_only});",
        f'{inner_indent}                _cf_pstmt{suffix}.setString(6, "{call_only}");',
        f"{inner_indent}                _cf_pstmt{suffix}.setLong(7, _cf_dur{suffix});",
        f"{inner_indent}                _cf_pstmt{suffix}.setBytes(8, _cf_serializedResult{suffix});",
        f'{inner_indent}                _cf_pstmt{suffix}.setString(9, "function_call");',
        f"{inner_indent}                _cf_pstmt{suffix}.setString(10, _cf_stdout{suffix});",
        f"{inner_indent}                _cf_pstmt{suffix}.executeUpdate();",
        f"{inner_indent}            }}",
        f"{inner_indent}        }}",
        f"{inner_indent}    }} catch (Exception _cf_e{suffix}) {{",
        f'{inner_indent}        System.err.println("CodeflashHelper: SQLite error: " + _cf_e{suffix}.getMessage());',
        f"{inner_indent}    }}",
        f"{inner_indent}}}",
        f"{indent}}}",
    ]
...
def _build_void_serialize_expr(call: dict[str, Any]) -> str:
    """Build a Serializer.serialize(...) expression for void function side-effect capture.

    For void methods, we serialize the arguments (and receiver for instance methods)
    AFTER the call, to capture any mutations as side effects.
    Static class receivers (uppercase first letter) are excluded since they aren't instances.
    """
    parts: list[str] = []
    receiver = call.get("receiver")
    arg_exprs = call.get("arg_exprs", [])
    if receiver and not (receiver[0].isupper() and receiver.isidentifier()):
        parts.append(receiver)
    parts.extend(arg_exprs)
    if not parts:
        return "null"
    items = ", ".join(parts)
    return f"com.codeflash.Serializer.serialize(new Object[]{{{items}}})"
...
def wrap_target_calls_with_treesitter(
    ...
    for line_idx, body_line in enumerate(body_lines):
        ...
        for call in line_calls:
            ...
            if is_void:
                capture_stmt_with_decl = f"{call['full_call']};"
                capture_stmt_assign = f"{call['full_call']};"
            else:
            ...
            if precise_call_timing:
                ...
                if is_void:
                    ser_expr = _build_void_serialize_expr(call)
                    serialize_stmt = f"_cf_serializedResult{iter_id}_{call_counter} = {ser_expr};"
                else:
                ...
                if is_void:
                    ser_expr = _build_void_serialize_expr(call)
                    serialize_stmt = f"_cf_serializedResult{iter_id} = {ser_expr};"
                else:
                    serialize_stmt = (
            ...
            if call["parent_type"] == "expression_statement":
                ...
                if precise_call_timing:
                    ...
                    # Declare per-call variables (skip result variable for void)
                    var_decls = [
                        ...
                        f"java.io.ByteArrayOutputStream _cf_stdoutCapture{iter_id}_{call_counter} = new java.io.ByteArrayOutputStream();",
                        f"java.io.PrintStream _cf_origOut{iter_id}_{call_counter} = System.out;",
                        f"String _cf_stdout{iter_id}_{call_counter} = null;",
                    ]
                    if not is_void:
                        var_decls.insert(0, f"Object {var_name} = null;")
                    # Start marker
                    ...
                    # Try block with capture and stdout redirection
                    try_block = [
                        "try {",
                        f"    System.setOut(new java.io.PrintStream(_cf_stdoutCapture{iter_id}_{call_counter}));",
                        f"    {start_stmt}",
                        f"    {capture_stmt_assign}",
                        f"    {end_stmt}",
                        f"    {serialize_stmt}",
                    ]
                    # Finally block with stdout restore and SQLite write
                ...
                # For void functions embedded in expressions, don't replace the call with a variable
                # (this case is unusual for void methods but handle it gracefully)
                if not is_void:
    ...
    if node_type == "method_invocation":
        ...
        if name_node and analyzer.get_node_text(name_node, wrapper_bytes) == func_name:
            ...
            if start >= 0 and end <= body_len:
                ...
                # Extract receiver and argument expressions for side-effect serialization
                object_node = node.child_by_field_name("object")
                receiver_text = analyzer.get_node_text(object_node, wrapper_bytes) if object_node else None
                args_node = node.child_by_field_name("arguments")
                arg_exprs = []
                if args_node:
                    for child in args_node.children:
                        if child.type not in ("(", ")", ","):
                            arg_exprs.append(analyzer.get_node_text(child, wrapper_bytes))
...
def instrument_generated_java_test(
    ...
    is_void = getattr(function_to_optimize, "return_type", None) == "void"
    test_code = transform_java_assertions(test_code, function_name, qualified_name, is_void=is_void)
