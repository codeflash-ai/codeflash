...
def _generate_sqlite_write_code(
    ...
    return [
        ...
        f'{inner_indent}        try (Connection _cf_conn{suffix} = DriverManager.getConnection("jdbc:sqlite:" + _cf_outputFile{iter_id})) {{',
        ...
        f'{inner_indent}                _cf_stmt{suffix}.execute("PRAGMA journal_mode=WAL");',
        f'{inner_indent}                _cf_stmt{suffix}.execute("PRAGMA busy_timeout=30000");',
        ...
        f"{inner_indent}                _cf_pstmt{suffix}.setString(1, _cf_mod{iter_id});",
        f"{inner_indent}                _cf_pstmt{suffix}.setString(2, _cf_cls{iter_id});",
        f"{inner_indent}                _cf_pstmt{suffix}.setString(3, _cf_test{iter_id});",
        f"{inner_indent}                _cf_pstmt{suffix}.setString(4, _cf_fn{iter_id});",
        f"{inner_indent}                _cf_pstmt{suffix}.setInt(5, _cf_loop{iter_id});",
        f'{inner_indent}                _cf_pstmt{suffix}.setString(6, "{call_counter}");',
