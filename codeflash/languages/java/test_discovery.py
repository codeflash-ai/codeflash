...
def discover_tests(
    ...
    Also handles replay test files (generated by the Java tracer) by parsing
    their metadata comments.
    ...
    test_files = (
        ...
        + list(test_root.rglob("Test*.java"))
        + list(test_root.rglob("ReplayTest_*.java"))
    )
    ...
    for test_file in test_files:
        try:
            # Check if this is a replay test file (has codeflash metadata)
            replay_metadata = _parse_replay_metadata(test_file)
            if replay_metadata:
                _discover_replay_tests(test_file, replay_metadata, function_map, result, analyzer)
                continue
...
def _parse_replay_metadata(test_file: Path) -> dict[str, str] | None:
    """Check if a test file is a replay test and extract its metadata.
...
def _discover_replay_tests(
    ...
    """Map replay test methods to source functions using metadata comments."""
    function_names = [f.strip() for f in metadata.get("functions", "").split(",") if f.strip()]
    test_methods = discover_test_methods(test_file, analyzer)

    # Extract test class name from the file
    test_class = None
    if test_methods:
        test_class = test_methods[0].class_name

    for test_method in test_methods:
        # Each replay test method is named replay_<methodName>_<index>
        # Map it to the source function it exercises
        for func_name in function_names:
            if func_name in function_map:
                qualified_name = function_map[func_name].qualified_name
                result[qualified_name].append(
                    TestInfo(
                        test_name=test_method.function_name,
                        test_file=test_file,
                        test_class=test_class or test_method.class_name,
                        is_replay=True,
                    )
                )

    logger.debug(
        "Discovered %d replay test methods for functions %s in %s", len(test_methods), function_names, test_file.name
    )
