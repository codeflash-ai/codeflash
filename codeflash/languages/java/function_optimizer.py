...
class JavaFunctionOptimizer(FunctionOptimizer):
    def try_correct_module_root(self) -> bool:
        """Validate module root and attempt auto-correction if invalid."""
        ...
        module_root = getattr(self.args, "module_root", project_root) or project_root

        valid, _ = validate_java_module_resolution(source_file, project_root, Path(module_root))
        if valid:
            return True

        inferred_root = infer_java_module_root(source_file, project_root)

        # Verify the inferred root actually contains the source file
        try:
            source_file.resolve().relative_to(inferred_root.resolve())
        except ValueError:
            return False

        # Update the config file on disk
        try:
            self._update_config_module_root(inferred_root)
        except Exception:
            logger.warning("Failed to update config file with inferred module root")

        # Update in-memory config
        self.args.module_root = inferred_root
        if hasattr(self.args, "project_root"):
            self.args.project_root = project_root
        self.project_root = project_root

        return True

    def _update_config_module_root(self, new_module_root: Path) -> None:
        """Update the module-root in codeflash.toml or pyproject.toml."""
        import tomlkit
        ...
        if config_path.name == "pyproject.toml":
            ...
            codeflash_section = tool.setdefault("codeflash", {})
        else:
            codeflash_section = doc

        codeflash_section["module-root"] = str(new_module_root)
        config_path.write_text(tomlkit.dumps(doc), encoding="utf-8")

    def can_be_optimized(self) -> Result[tuple[bool, CodeOptimizationContext, dict[Path, str]], str]:
        from codeflash.code_utils.config_java_validation import validate_java_module_resolution
        ...
        valid, error = validate_java_module_resolution(source_file, project_root, Path(module_root))
        if not valid:
            return Failure(f"Java module validation failed: {error}")
