...
def test_optimize_help_displays_logo_once() -> None:
    result = subprocess.run(
        [sys.executable, "-c", "from codeflash.main import main; main()", "optimize", "--help"],
        capture_output=True,
        text=True,
        encoding="utf-8",
    )
    assert result.returncode == 0
    assert result.stdout.count("codeflash.ai") == 1
