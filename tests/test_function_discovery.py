...
def test_find_top_level_function_or_method(temp_dir: Path):
    temp_path = temp_dir / "test_file.py"
...
def non_classmethod_function(cls, name):
    ...
    temp_path_invalid = temp_dir / "invalid.py"
    temp_path_invalid.write_text(
        """def functionA():
"""
    )
    path_obj_name = temp_path_invalid
    assert not inspect_top_level_functions_or_methods(path_obj_name, "functionA")

def test_class_method_discovery(temp_dir: Path):
    temp_path = temp_dir / "test_class_method.py"
    temp_path.write_text(
        """class A:
    def functionA():
        return True
...
def test_nested_function(temp_dir: Path):
    temp_path = temp_dir / "nested1.py"
    temp_path.write_text(
"""
import copy 
...
"""
    ...
    temp_path2 = temp_dir / "nested2.py"
...
"""
    ...
    temp_path3 = temp_dir / "nested3.py"
...
def test_filter_files_optimized(temp_dir: Path):
    tests_root = temp_dir / "tests"
    tests_root.mkdir(exist_ok=True)
    module_root = temp_dir
    ignore_paths = []

    file_path_test = tests_root / "test_function_discovery.py"
    file_path_test.touch()
    file_path_same_level = temp_dir / "file.py"
    file_path_same_level.touch()
    file_path_different_level = temp_dir / "src" / "file.py"
    file_path_different_level.parent.mkdir(exist_ok=True)
    file_path_different_level.touch()
    file_path_above_level = temp_dir.parent / "file.py"
    file_path_above_level.touch()
...
def test_filter_functions(temp_dir: Path):
