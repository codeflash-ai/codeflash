...
# ============================================================================
# Device Synchronization Tests for Async Functions
# ============================================================================


@pytest.mark.skipif(sys.platform == "win32", reason="pending support for asyncio on windows")
def test_async_profiling_does_not_use_device_sync(temp_dir):
    """Async profiling currently does not support device sync (uses decorator approach)."""
    source_module_code = '''
import asyncio
...
'''
    ...
    # Async functions use inject_async_profiling_into_existing_test which doesn't support used_frameworks yet
    # This test documents the current behavior
    success, instrumented_test_code = inject_async_profiling_into_existing_test(
        test_file, [CodePosition(9, 18)], func, temp_dir, mode=TestingMode.BEHAVIOR
    )

    # Async profiling should succeed
    assert success
    assert instrumented_test_code is not None
    # But it should NOT have device sync code (async uses decorator approach)
    assert "codeflash_torch" not in instrumented_test_code
    assert "codeflash_tf" not in instrumented_test_code
    assert "codeflash_jax" not in instrumented_test_code


@pytest.mark.skipif(sys.platform == "win32", reason="pending support for asyncio on windows")
def test_inject_profiling_with_async_function_ignores_frameworks(temp_dir):
    """When function is async, inject_profiling_into_existing_test delegates to async version which ignores frameworks."""
    source_module_code = '''
import asyncio
...
'''
    ...
    # Even with used_frameworks specified, async functions don't use device sync
    # because they use the decorator approach
    success, instrumented_test_code = inject_profiling_into_existing_test(
        test_file,
        [CodePosition(9, 18)],
        func,
        temp_dir,
        mode=TestingMode.BEHAVIOR,
        used_frameworks={"torch", "tensorflow", "jax"},  # These should be ignored for async
    )

    # Async profiling should succeed
    assert success
    assert instrumented_test_code is not None
    # Device sync code should NOT be present (async functions use decorator approach)
    assert "codeflash_torch" not in instrumented_test_code
    assert "codeflash_tf" not in instrumented_test_code
    assert "codeflash_jax" not in instrumented_test_code
